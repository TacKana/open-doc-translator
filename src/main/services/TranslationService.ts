...
export class TranslationService extends EventEmitter {
  ...
  async translateSingleFile(
    ...
    try {
      ...
      // 执行翻译
      const result = await this.llmManager.translateFile(
        filePath,
        content,
        translationPrompt,
        sourceHash,
   ...
   * 批量翻译文件
   */
  async batchTranslateFiles(filePaths: string[], prompt?: string): Promise<TranslationResult[]> {
    if (!this.currentProjectId || !this.llmManager) {
      throw new Error('未选择项目或LLM未初始化')
    }
    ...
    try {
      ...
      // 执行批量翻译
      const results = await this.llmManager.translateBatch(tasks, translationPrompt, (progress) => {
